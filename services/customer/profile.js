...
exports.updateRegToken = async function (userId, regToken) {
  if (!regToken) throw new AppError().GENERIC_ERROR('Invalid token');
...
exports.userInformationService = async function (userId) {
  ...
  if (user.userRole !== 'customer')
    throw new AppError().GENERIC_ERROR('Incorrect credentials');
...
function getTaskCounts(artisanServices) {
  ...
  let scheduledTasCount = 0;
  let completedTaskCount = 0;
  for (let artisanService of artisanServices) {
    for (let task of artisanService.tasks) {
      if (task.jobStatus === 'customerConfirmed') completedTaskCount++;
      else if (
        task.jobStatus === 'completed' ||
        task.jobStatus === 'artisanArrived'
      )
        ongoingTaskCount++;
      else if (task.jobStatus === 'accepted') scheduledTasCount++;
    }
  }
...
async function getValidUser(userId) {
  ...
  if (user.userRole !== 'customer')
    throw new AppError().GENERIC_ERROR('Invalid credentials');
